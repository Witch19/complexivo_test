...
from .mongo_serializers import LabTestsTypeSerializer

col = db["labTests_types"] #menus_types
...
def labTests_types_list_create(request):
    ...
    serializer = LabTestsTypeSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def labTests_types_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = LabTestsTypeSerializer(data=request.data, partial=(request.method == "PATCH")) 
        serializer.is_valid(raise_exception=True)
